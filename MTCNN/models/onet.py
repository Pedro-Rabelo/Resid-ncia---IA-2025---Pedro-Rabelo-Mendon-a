...
class ONet(nn.Module):
    """
    Output Network (O-Net) - Terceiro estágio do MTCNN
    
    Arquitetura:
    - Input: 48x48x3
    - Conv1: 32 filtros 3x3, stride=1
    - PReLU + MaxPool 3x3, stride=2
    - Conv2: 64 filtros 3x3, stride=1
    - PReLU + MaxPool 3x3, stride=2
    - Conv3: 64 filtros 3x3, stride=1
    - PReLU + MaxPool 2x2, stride=2
    - Conv4: 128 filtros 2x2, stride=1
    - PReLU
    - FC1: 256 units
    - PReLU
    ...
    - Landmark localization: 10 valores (precisão aumentada)
    """
    ...
    def __init__(self):
        ...
        # Calcular dimensão: 48 -> 46 -> 23 -> 21 -> 10 -> 8 -> 4 -> 2 -> 1
        # FC input: 128 * 3 * 3 = 1152
        ...
        # Task 3: Facial landmark localization (PESO MAIOR)
    ...
    def forward(self, x):
        ...
        """
        # Convolutional layers
        x = self.conv1(x)      # [B, 32, 46, 46]
        x = self.prelu1(x)
        x = self.pool1(x)      # [B, 32, 23, 23]
        
        x = self.conv2(x)      # [B, 64, 21, 21]
        x = self.prelu2(x)
        x = self.pool2(x)      # [B, 64, 10, 10]
        
        x = self.conv3(x)      # [B, 64, 8, 8]
        x = self.prelu3(x)
        x = self.pool3(x)      # [B, 64, 4, 4]
        
        x = self.conv4(x)      # [B, 128, 3, 3]
        x = self.prelu4(x)
...
class ONetLoss(nn.Module):
    """
    Multi-task Loss para O-Net
    PESO MAIOR para landmarks (1.0 vs 0.5 nos outros)
    """
    ...
    def __init__(self, cls_weight=1.0, box_weight=0.5, landmark_weight=1.0):
        ...
        self.landmark_weight = landmark_weight  # MAIOR que P-Net e R-Net
    ...
    def forward(self, cls_pred, box_pred, landmark_pred,
        ...
        # Landmark Loss (landmark only) - PESO MAIOR
        ...
        # Total Loss com peso maior para landmarks
...
if __name__ == "__main__":
    ...
    print(f"\nNote: Landmark weight = {loss_fn.landmark_weight} (maior que P-Net e R-Net)")
    print("\n✓ O-Net testado com sucesso!")