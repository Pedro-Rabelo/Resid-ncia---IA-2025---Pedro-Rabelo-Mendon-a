...
from models.losses import ONetLoss
...
def train_onet():
    """Treina O-Net com peso MAIOR para landmarks"""
    print("\n" + "="*70)
    print("TREINAMENTO DO O-NET")
    print("="*70)
    ...
    # Dataset
    train_anno_file = os.path.join(Config.PROCESSED_DATA_DIR, 'onet', 'onet_train.txt')
    
    if not os.path.exists(train_anno_file):
        print(f"\n❌ Erro: {train_anno_file} não encontrado")
        print("Execute: python main.py --mode prepare --stage onet")
        return
    
    print(f"\n[Dataset] Carregando dados...")
    train_dataset = MTCNNDataset(
        annotation_file=train_anno_file,
        input_size=Config.ONET_INPUT_SIZE
    )
    
    train_loader = DataLoader(
        train_dataset,
        batch_size=Config.ONET_BATCH_SIZE,
        shuffle=True,
    ...
    # Model
    print(f"\n[Model] Inicializando O-Net...")
    model = ONet().to(device)
    print(f"  Parâmetros: {sum(p.numel() for p in model.parameters()):,}")
    
    # Loss e Optimizer - PESO MAIOR PARA LANDMARKS!
    loss_fn = ONetLoss(
        cls_weight=Config.ONET_CLS_WEIGHT,
        box_weight=Config.ONET_BOX_WEIGHT,
        landmark_weight=Config.ONET_LANDMARK_WEIGHT  # 1.0 vs 0.5
    )
    
    print(f"\n[Loss] Landmark weight: {Config.ONET_LANDMARK_WEIGHT} (MAIOR!)")
    
    optimizer = optim.Adam(model.parameters(), lr=Config.ONET_LR)
    ...
    # TensorBoard
    writer = SummaryWriter(os.path.join(Config.TENSORBOARD_DIR, 'onet'))
    
    # Training loop
    print(f"\n[Training] Iniciando...")
    print(f"  Epochs: {Config.ONET_EPOCHS}")
    print("-"*70)
    
    best_loss = float('inf')
    global_step = 0
    
    for epoch in range(Config.ONET_EPOCHS):
        model.train()
        epoch_losses = {'total': [], 'cls': [], 'box': [], 'landmark': []}
        
        pbar = tqdm(train_loader, desc=f"Epoch {epoch+1}/{Config.ONET_EPOCHS}")
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            pbar.set_postfix({
                ...
                'lmk': f"{loss_dict['landmark']:.4f}"  # Destacar landmark
            })
        ...
        print(f"\n[Epoch {epoch+1}] Total: {avg_losses['total']:.4f}, "
              f"Landmark: {avg_losses['landmark']:.4f}")
        
        scheduler.step()
        
        if (epoch + 1) % Config.SAVE_INTERVAL == 0 or avg_losses['total'] < best_loss:
            checkpoint_path = os.path.join(
                Config.CHECKPOINT_DIR, 'onet', f'onet_epoch_{epoch+1}.pth'
            )
            torch.save(model.state_dict(), checkpoint_path)
            
            if avg_losses['total'] < best_loss:
                best_loss = avg_losses['total']
                best_path = os.path.join(Config.CHECKPOINT_DIR, 'onet', 'onet_best.pth')
                torch.save(model.state_dict(), best_path)
                print(f"  ✓ Best model: {best_loss:.4f}")
        
        print("-"*70)
    
    final_path = Config.ONET_CHECKPOINT
    torch.save(model.state_dict(), final_path)
    print(f"\n[Finalizado] Model salvo: {final_path}")
    print(f"[Finalizado] Best loss: {best_loss:.4f}")
    
    writer.close()
    print("\n✓ TREINAMENTO DO O-NET CONCLUÍDO!")
